...
export interface VocabFormCardProps {
...
export function VocabFormCard({ vocab, onSuccess, onCancel }: VocabFormCardProps) {
  ...
  const onSubmit: SubmitHandler<CreateVocabInput> = async (values) => {
    try {
      if (isEdit && vocab) {
        ...
        toast.success(`"${vocab.word}" updated.`)
        onSuccess(vocab.id)
      } else {
        const created = await createVocab(values)
        toast.success(`"${created?.word ?? values.word}" added.`)
        reset({ word: '', content: '' })
  ...
  return (
    <div className="rounded-lg border border-[#e1dfdd] bg-white shadow-sm">
      {/* Card header */}
      <div className="border-b border-[#e1dfdd] px-6 py-4">
        <h2 className="text-[15px] font-semibold text-[#323130]">
          {isEdit ? `Edit — ${vocab!.word}` : 'New Word'}
        </h2>
        {stageInfo && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 px-6 py-5">
        ...
        <div className="flex items-center justify-between border-t border-[#e1dfdd] pt-4">
          <div>
            {isEdit && (
              <button
                type="button"
