...
/* 
*************************************************************************************************
* @source  : GMES_IM_POM/GMES_IMES_0272.aspx
* @desc    : [일지관리] 양극제 일지 관리
************************************************************************************************* 
...
* 1.3     2026.01.21    김정태        모의해킹 방어로직( 엑셀 다운로드 방식 변경 )                              
*************************************************************************************************
*/

public partial class GMES_IMS_0272 : GMESPage
{
    ...
    private void ParsingRequest()
    {
        ...
        hidAuthID.Value = Request["AUTHID"].ToString();  //2026.02.03 황유라 주석 제거
        hidMenuID.Value = Request["MENU_ID"].ToString(); //2026.02.03 황유라 주석 제거
    ...
    //       catch (Exception ex)
    //       {
    //           rtn.message = "[GMES_IMES_0272] DownloadExcelImage exception occurred";
    //           rtn.status = "FAIL";
    //       }
    //
    //       return rtn;
    ...
    [WebMethod(EnableSession = true), ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static ApiResponse DownloadExcelImage(
                                                    string bizID,
                                                    object[] items,
                                                    string inTableNames,
                                                    string outTableNames,
                                                    string dyrdNm,
                                                    string templateNm,
                                                    string labelCnt,
                                                    string weekName,
                                                    string lastDay)
    {
        ...
        try
        {
            DataSet dataSet = FillBizActor_ReturnByDataSet(
                bizID, items, inTableNames, outTableNames);

            string fileName = string.Format(
                "DailyRecord_{0}_{1}_{2}.xlsx",
                SafeFileName(dyrdNm),
                SafeFileName(templateNm),
            ...
            // 🔐 실제 경로는 Session에만 저장
            HttpContext.Current.Session["EXCEL_FILE_" + fileToken] = strFullPath;

            // ❗ Response에는 절대경로 대신 Token만 전달
            rtn.data = fileToken;
            rtn.status = "OK";
        }
        catch (Exception)
    ...
    public static void DownloadExcelFile(string token)
    {
        string key = "EXCEL_FILE_" + token;
        string filePath = HttpContext.Current.Session[key] as string;
        ...
        response.AddHeader(
            "Content-Disposition",
            "attachment; filename=DailyRecord.xlsx");
    ...
    private static string GetNextWeekName(string curWeekName)
    {
        ...
        string[] arrWeekName_Tue = { "화", "Tue", "星期二", "星期二" };
        string[] arrWeekName_Wed = { "수", "Wed", "星期三", "星期三" };
